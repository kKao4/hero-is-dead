...
export default function HeaderDecoTopBottom({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex flex-col mx-auto w-fit gap-y-2 md:gap-y-4 items-center">
      <Image src={decoTop} alt="" className="h-8 sm:h-10 lg:h-14" quality={0} priority={false} />
      {children}
      <Image src={decoBottom} alt="" className="h-8 sm:h-10 lg:h-14" quality={0} priority={false} />
    </div>
  )
}