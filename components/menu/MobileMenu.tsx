  ...
  return (
    <>
      ...
      <div className="fixed z-50 block w-8 animate__animated animate__fadeInDown top-3 right-3 h-9 lg:hidden" onClick={() => setIsOpenResponsiveMenu((prevState: any) => !prevState)}>
        <div className={`${isOpenResponsiveMenu ? "rotate-[44deg] translate-y-[9px] w-[33px]" : "rotate-0 w-8"} transition-transform top-[7px] duration-350  ease-out h-[3px] absolute bg-black`} />
        ...
        <div className={`${isOpenResponsiveMenu ? "-rotate-[44deg] -translate-y-2.5 w-[33px]" : "rotate-0 w-8"} h-[3px] transition-transform duration-350 ease-out absolute bottom-[6px] bg-black`} />
      </div>
    </>
  )
}